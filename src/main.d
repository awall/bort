...
int myWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int iCmdShow) {
  ...
  hWnd = CreateWindowW(
    className.ptr,       // window class name
    caption.ptr,         // window caption
    WS_THICKFRAME | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_SYSMENU | WS_VISIBLE, // window style
    CW_USEDEFAULT,       // initial x position
    CW_USEDEFAULT,       // initial y position
    600, 400,            // initial x,y size
      ...
      switch (LOWORD(wParam)) {
        ...
        case IDC_BTNCLICK:
          if (HIWORD(wParam) == BN_CLICKED)
            MessageBoxW(hWnd, "Hello, world!", "Greeting", MB_OK | MB_ICONINFORMATION);
          break;

        case IDC_BTNDONTCLICK:
          if (HIWORD(wParam) == BN_CLICKED) {
            MessageBoxW(hWnd, "You've been warned...", "Prepare to GP fault", MB_OK | MB_ICONEXCLAMATION);
            *gp = 1;
          }
          break;
      }
